...
wgcna = list.files(path = "output_sct_reclust/",pattern = 'SalineCPP_Markers_Reclust_Stats_DGE.txt',full.names = TRUE)
...
load("utils/GeneSets_NAc_CellReport.RData")
...
write.table(tmp2,"output_sct_reclust/Labels_Clusters.txt",sep="\t",quote=F)
...
pdf("output_sct_reclust/NAc_CellRep_Enrichment.pdf",width=18,height=8)
ggscatter(p, 
        x = "variable",
        y = "Module",
        size="OR",
